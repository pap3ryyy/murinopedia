...
код — говнокод. проект легендарный но поддерживать я его больше не буду (но не факт), но вы можете поднять копию. Местами будет больно, но оно работает. (если нихуя не понятно спросите нейронку мне просто похуй ну или напишите в личку или в приватный чат муринопедии если имеете я вам помогу)
...
### 2. Создайте проект в Supabase и на github

Заходите на supabase, жмёте New project, даёте имя, придумываете пароль базы, выбираете ближайший регион. Ждёте пару минут.

на гитхабе созданьте репу и залейте туда код (там будете все менятьь в settings можете включить pages если вам нужно видеть сайт)
---
...
### 4.5. Хранилище файлов (картинки, треки)

Сайт загружает аватарки, обложки, значки и музыку напрямую в Supabase Storage. Нужно создать два публичных бакета и разрешить запись авторизованным.

1. В левом меню Supabase → **Storage**.
2. Нажми **New bucket**:
   - Имя: `images`, включай тумблер **Public bucket** → Create.
   - Снова **New bucket**: имя `tracks`, тоже **Public bucket** → Create.
3. Настрой политики:
   - В Storage выбери бакет `images` → вкладка **Policies**.
   - Нажми **New policy** → для `INSERT` выбери шаблон `Give users access to upload files` (или создай свою: целевая роль `authenticated`, условие `true`). Сохрани.
   - Если нет политики для `SELECT` — создай ещё одну: `SELECT` → разрешить всем (`true`).
   - Повтори то же самое для бакета `tracks` (INSERT для authenticated, SELECT для всех).
4. Папки `avatars`, `banners`, `backgrounds`, `badges`, `covers`, `content`, `tracks` создавать вручную не нужно — они появятся сами при первой загрузке файла. Но если хочется порядка, создай их через кнопку **New folder** в каждом бакете.

Готово. Теперь загрузка изображений и треков работает.
...
Зайди в настройки репозитория → **Settings** → **Pages**.
В разделе «Branch» выбери `main`, папка `/ (root)`, нажми **Save**.
Через пару минут GitHub выдаст ссылку типа `https://твой-юзернейм.github.io/название-репозитория`. Это и есть сайт.
Эту ссылку вставь в Supabase в **Authentication → Settings → Site URL**, а также добавь в **Redirect URLs** с `/**` (например, `https://твой-юзернейм.github.io/название-репозитория/**`).
...
также пожалуйста запомните вам нужно улучшить кэширование иначе от того что картинки с базы будут грузиться очень часто база просто упадет от лимита на некоторое время, это не обязательно для работы, также нужен актив если на базе не будет изменений она упадет.
